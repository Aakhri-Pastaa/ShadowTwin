# go-agent-v0 — Development Log (archived)

> **Archived.** This is the development log for the original custom Go
> telemetry agent, preserved as history. The project pivoted to a
> **Wazuh-based** telemetry/detection source; see the root
> [`DEVLOG.md`](../DEVLOG.md) for the current project's log and
> [`README.md`](README.md) for the archive note. Everything below predates
> the pivot and describes the agent **as it stood when archived** — it is
> not the current state of the project.

A running, human-readable record of how this component was built: what we
did, why we did it, how it works, and how each step was verified. Format:
one dated entry per meaningful chunk of work, shaped **What · Why · How ·
Verification · Refs**, oldest first.

---

## Project at a glance (at time of archiving)

**ShadowTwin** was, at this point, a two-person, open-source, closed-loop
purple-team lab built around a custom **host agent**: it collects telemetry
from a deliberately vulnerable sandbox; an Evaluator triages it; an
Attacker validates exploitability *inside the sandbox only*; a Defender
remediates and re-triggers the Attacker to prove the fix.
...
**Where we ended up (before the Wazuh pivot):** the **host agent** had a
complete, end-to-end **telemetry shipping path**: it collects authentication
events from `systemd-journald`, persists them to a durable on-disk buffer,
and ships them to the platform over mutually authenticated TLS, enrolling
for its client certificate on first run. It was a single static Go binary
with **zero third-party dependencies**. The other components (graph,
threat-intel, evaluator, attacker, defender, frontend) were never started
against this agent — the project pivoted to Wazuh before they landed.

---

## Final state — what worked when archived
...
| Real platform / ingest server | ⛔ | Never built; mock stood in. Contract in ADR-0003 |
...
**Refs.** PR #1 → `20e690c`. README: `go-agent-v0/README.md`.
...
has no `go.mod` (the module lives in `host-agent/`, now `go-agent-v0/`), so
commits failed with "cannot find main module". The `go-vet-mod` variant belongs
to a *different* fork (TekWizely), not the one we pin — so instead of adding a
third-party fork we used a `local` hook that runs `go vet ./...` inside the
module directory, mirroring CI's working directory.
...
### 2026-07-02 — Archived: pivot to Wazuh

**What.** The agent was renamed `host-agent/` → `go-agent-v0/`, marked
archived, and superseded by Wazuh as the project's telemetry/detection
...
## Backlog at time of archiving (not pursued further here)

This was the rough priority order for the host agent when the project
pivoted; it is not being worked going forward, kept for the record:
